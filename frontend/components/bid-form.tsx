...
import { createBid } from "@/lib/api"; // Added
...
export function BidForm({ artwork, minBid }: BidFormProps) {
  ...
  const isAuctionEnded = new Date() > artwork.end_time;
  const current_bidAmount = Number.parseFloat(bidAmount) || 0;
  const isValidBid = current_bidAmount >= minBid;
  const canSubmitBid = isValidBid && !isSubmitting && !isAuctionEnded;

  const quickBidAmounts = [
    minBid,
    minBid + artwork.min_increment,
    minBid + artwork.min_increment * 2,
    minBid + artwork.min_increment * 5,
  ];
  ...
  const adjustBid = (increment: number) => {
    const newAmount = Math.max(minBid, current_bidAmount + increment);
  ...
  const handleSubmitBid = async (e: React.FormEvent) => {
    ...
    try {
      await createBid(artwork.id, current_bidAmount); // Updated to use API
      setShowConfetti(true);
      toast({
        title: "Bid Placed Successfully!",
        description: `Your bid of ${formatPrice(
          current_bidAmount
        )} has been placed.`,
      });
      ...
      <Card>
        ...
        <CardContent>
          <Alert severity="info">
            ...
            {formatPrice(artwork.current_bid)} by{" "}
            {artwork.highest_bidder || "Anonymous"}.
          </Alert>
        </CardContent>
      </Card>
    );
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                <Button
                  ...
                  variant="outlined"
                  size="small"
                  onClick={() => adjustBid(-artwork.min_increment)}
                  disabled={current_bidAmount <= minBid}
                  className="px-3"
                >
                ...
                <div className="relative flex-1">
                  ...
                  <TextField
                    ...
                    inputProps={{ min: minBid, step: artwork.min_increment }}
            ...
            <div className="p-4 rounded-lg bg-muted/50 space-y-2">
              <div className="flex justify-between text-sm">
                ...
                <span className="font-semibold">
                  {formatPrice(current_bidAmount)}
                </span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-muted-foreground">Current high bid:</span>
                <span>{formatPrice(artwork.current_bid)}</span>
              </div>
              <div className="flex justify-between text-sm">
                ...
                <span className="font-semibold text-primary">
                  +
                  {formatPrice(
                    Math.max(0, current_bidAmount - artwork.current_bid)
                  )}
                </span>
              </div>
            </div>
            ...
            >
              {isSubmitting ? (
                ...
                <>
                  ...
                  Place Bid {formatPrice(current_bidAmount)}
                </>
              )}
            </Button>
